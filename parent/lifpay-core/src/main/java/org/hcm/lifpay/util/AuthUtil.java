...
import org.hcm.lifpay.exception.NoPermissionException;
...
public class AuthUtil {
    ...
    public static void checkUserType(RequiresUserType requiresUserType) {
        ...
        // 从local获取用户信息
        UserInfo adminUserInfo = getUserInfoLocal();
    ...
    public static void checkPermission(RequiresPermission requiresPermission) {
        ...
        // 从local获取用户信息
        UserInfo userInfo = getUserInfoLocal();
        // 正常是从网关过来的，都会有用户信息，如果没有，可能是swagger直接调的
        if (userInfo == null) {
            // 如果开启了免登录的配置，跳过
            if (authConfig.isNoAuth()){
        ...
        if (AdminUserTypeEnum.ADMIN.getCode() != userInfo.getUserType().getCode()) {
            boolean userHashPermission = hashPermission(checkUserPermission, userInfo.getUserPermission());
    ...
    public static UserInfo getUserInfoLocal() {
        return SecurityContextHolder.get(Constants.THREAD_LOCAL_KEY_CURRENT_USER, UserInfo.class);
    }

    public static void setUserInfoLocal(UserInfo userInfo) {
        SecurityContextHolder.set(Constants.THREAD_LOCAL_KEY_CURRENT_USER, JSON.toJSONString(userInfo));
    }


    public static UserInfo getUserInfoCache(Long userId) {
        // 从redis获取
    ...
    public static void setUserInfoCache(UserInfo userInfo, int expire) {
